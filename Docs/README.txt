战略指挥部 / Strategic Command
================================

《Sid Meier's Civilization V》后期高层托管与微操减负 Mod。

当前版本：v1.37
状态：Alpha
支持：单人游戏
不支持：多人、热座

核心目标
--------

这个 Mod 用于减少文明 5 后期的重复操作：

- 城市生产队列反复需要补。
- 大量单位每回合需要命令。
- 科技、政策、意识形态、伟人、宗教、考古、世界议会和通知弹窗频繁打断。
- 战争中远程单位、舰队、空军、伤兵和城市炮击需要大量微操。

战略指挥部提供一个“托管若干回合”的高层指挥面板。玩家先选择外交、经济、建设、生产、战争等倾向，然后让脚本接管 5、10、20 或 50 回合。

主要功能
--------

1. 托管面板

- 左上角显示“战略”按钮。
- 支持 5 / 10 / 20 / 50 回合托管。
- 支持手动执行一次完整托管。
- 支持停止托管。
- UI 文本为中文。

2. 战略预设

- 外交：均衡、友好、强硬、孤立。
- 经济：均衡、科研、财政、扩张。
- 建设：基建、幸福、科研、防御。
- 开发：自动、产能、铁路、保守。
- 生产：建筑、军队、海空、奇观。
- 战争：防守、推进、总攻、海权。
- 占城：傀儡、智能、吞并、焚城。
- 伟人：保留、科研、工程、文化。
- 宗教：产能、科研、金币、文化。
- 贸易：均衡、金币、科研、内运。
- 间谍：反谍、窃技、城邦、外交。

3. 城市与生产

- 自动为城市补充生产队列。
- 根据战争状态、建筑类型、维护费、预设倾向评分。
- 避免同一轮给同一城市重复加入同一种建筑或单位。
- 支持城市炮击。

4. 科技、政策与意识形态

- 自动选择科研。
- 自动选择社会政策。
- 自动处理意识形态。
- 自动处理免费政策和相关通知。

5. 单位控制

- 自动升级单位。
- 自动选择晋升。
- 自动治疗伤兵。
- 自动处理需要命令的单位。
- 自动处理堆叠单位。
- 自动执行远程射击和城市炮击。
- 对空军、导弹、海军和陆地远程单位设置不同战术行动上限。
- 根据游戏数据库中的单位属性和初始晋升，将全部单位自动归入 31 类战术教义。
- 按制空/屏卫、纵深打击、远程压制、突击收城顺序行动，并共享战区目标。
- 航母、武库舰和弹道导弹潜艇后置，防空屏卫和护航舰保护高价值单位。
- 支持空中扫荡、炮兵架设、空降、安全航点、受损撤退和多次攻击。
- 远洋运输在威胁区内至少需要两艘护航单位才会继续推进。
- 托管期间拒绝移动、远程攻击或劫掠触发的所有新宣战确认。

6. 弹窗与通知

托管期间会尝试处理：

- 占领城市处置。
- 伟人诞生与免费伟人。
- 科技完成提示。
- 政策与意识形态提示。
- 宗教、万神殿、考古。
- 世界议会提案与投票。
- 外交领袖打断。
- 部分奇观、通知和低价值提示。

7. 自动结束回合

- 阻塞项清理后自动结束回合。
- 结束回合发送后进入转场静默模式，避免在 AI 回合切换阶段重复下命令。
- 日志记录结束回合阻塞、重试和静默等待原因。

安装
----

将目录放到：

Documents\My Games\Sid Meier's Civilization 5\MODS\Strategic Command (v 1)

游戏中：

1. 进入 Mods。
2. 勾选“战略指挥部”。
3. 载入或开始单人游戏。
4. 进入游戏后点击左上角“战略”按钮。

配置
----

配置文件：

Lua\StrategicCommand_Config.lua

常用开关：

- AutoCityProduction：自动城市生产。
- AutoResearch：自动科研。
- AutoPolicy：自动政策。
- AutoPromoteUnits：自动晋升。
- AutoStrategicMove：自动战略机动。
- AutoPopupHandling：自动弹窗处理。
- AutoEndTurn：自动结束回合。
- DebugLogging：调试日志。
- DebugUnitCommands：单位命令日志。
- DebugCityProduction：城市生产评分日志。

日志与测试
----------

日志会输出 [Strategic Command][SCDBG] 前缀，用于分析托管行为。

辅助脚本位于：

C:\Users\meade\Documents\文明mod

运行测试：

powershell -ExecutionPolicy Bypass -File "C:\Users\meade\Documents\文明mod\Run-StrategicCommandTests.ps1" -SkipLog

收集日志：

powershell -ExecutionPolicy Bypass -File "C:\Users\meade\Documents\文明mod\Collect-StrategicCommandLog.ps1"

已知限制
--------

- 当前仍是 Alpha 阶段，策略质量还在持续迭代。
- 部分 Civ5 UI 弹窗和单位命令缺少稳定公开 API，需要 fallback 处理。
- v1.31 加强后期战争 doctrine：航母后置，武库舰远程压制，052D/基洛夫等屏卫舰负责护航、清海军威胁和收城，潜艇猎杀敌舰，舰载机/导弹优先清理运输线与舰队威胁。
- v1.37 识别全球唯一精英工程及其解锁单位，自动抢建仍具时代价值的工程，并在完成后优先补造对应精英单位；限制并行工程和单位数量，默认排除末日机甲。
- v1.36 将斩首策略限制为有占城单位集结的局部战役，禁止继续轰炸无人收城的残血城市和负分目标，修复自定义时代识别及低级兵种误生产，并清理 v1.35 遗留错误队列。
- v1.35 将战时生产升级为多兵种打击包，独立补齐舰载机、轰炸机、舰队火力、航母、战略潜艇和导弹，并让编组共享斩首目标。
- v1.34 重写威胁驱动的运输护航和编制缺口驱动的战时生产，加入平民海运撤退、跨城唯一建筑预约、队列军事配额，并修复远距离安全航点误拒。
- v1.33 修复数据库布尔字段和移动工具作用域导致的整轮托管中断，加入策略模块故障隔离，并在原版领袖界面入队前拒绝 AI 主动外交窗口。
- v1.32 加入数据库驱动的全单位能力画像、31 类战术教义、战区集火、空战/空降/架设/撤退/编队/运输链路，以及读取实际单位数据库的轻量策略模拟器。
- 航母、舰载机、导弹、远洋舰队和复杂攻城链路仍是后续重点优化方向。
- 托管目标是减少微操，不保证每个局部操作都优于玩家手动控制。

作者
----

- meade：需求、测试、玩法设计。
- Codex：Lua 实现、调试与文档。
